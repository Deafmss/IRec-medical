...
- **Interface do Paciente Dinâmica (`ProtocolGuide.jsx`):** O paciente visualiza nos seus guias e protocolos de autocuidado uma interface totalmente dinâmica, limpa e estruturada:
  1. *Novo Layout de Grid de 2 Colunas:* Adotamos o design visual limpo e moderno dos cards de "Médicos Especialistas" para a listagem das instruções de curativo e para os insumos sugeridos. Eles agora se organizam em uma grade de duas colunas responsivas, contendo avatares numéricos/ícones, cabeçalhos informativos claros e rodapés de status/redirecionamento.
  2. *Renomeação da Seção:* Simplificamos o título da seção de instruções passo a passo para *"Guia Instruções de Autocuidados"* (ou *"Condutas e Diretrizes Clínicas de Manejo"* na visão médica), tornando-o mais direto.
  3. *Indicador Visual de Tarja do Medicamento:* Exibimos uma tarja colorida em formato de barra em cada card de insumo simulando a embalagem real:
     * **🔴 Tarja Vermelha / Tarja Vermelha (Retenção):** Para antifúngicos orais (comprimidos, retenção obrigatória) e de uso tópico regulado (cremes/shampoos).
     * **🌿 Venda Livre / MIP:** Para insumos de venda livre e cuidados básicos (sabonetes neutros, ataduras, soro).
  4. *Alerta de Receita Médica:* Banners chamativos de aviso em tom âmbar (*"📄 Necessita de receita médica..."*) são renderizados logo abaixo da descrição do insumo para orientar o paciente de forma clara caso o produto exija prescrição.
  5. *Condições de Exibição dos Parceiros do Médico:* Botões e farmácias indicadas pelo médico só aparecem caso os médicos responsáveis pelo paciente tenham de fato parcerias cadastradas. Caso contrário, nenhum botão de redirecionamento ou compra é exibido.
  6. *Condições de Exibição dos Parceiros iRec:* Os parceiros iRec oficiais são exibidos de forma completamente isolada em sua própria seção dedicada ("Parceiros iRec"), e não mais misturados dentro dos cards de sugestões da IA ou protocolos padrão.
  7. *Layout Compacto e Remoção de Preços:* Reduzimos significativamente o padding e o espaçamento dos cards de recomendação da IA e protocolos padrão para encurtar a rolagem da página. Adicionalmente, removemos totalmente o campo de valor do insumo recomendado nesses cards.
  8. *Fallback da Farmácia Mais Próxima:* Caso os médicos responsáveis por aquele paciente não tenham parceiros cadastrados, o card exibe uma nota informativa instruindo o paciente a procurar a farmácia credenciada mais próxima para aquisição do produto (substituindo o antigo aviso de consulta ao administrador).
  9. *Modal de Redirecionamento com Contagem Regressiva:* Como a plataforma iRec não realiza vendas diretas, removemos o fluxo de "Pedido Confirmado / iRec Delivery" e o substituímos por uma tela de redirecionamento segura com contagem regressiva de 3 segundos (e redirecionamento automático) que exibe o nome do insumo, o parceiro/farmácia de destino e o nome do médico autor (ou indicação oficial iRec). E-commerce simulado desativado.
