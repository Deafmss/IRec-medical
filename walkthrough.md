...
- **Interface do Paciente Dinâmica (`ProtocolGuide.jsx`):** O paciente visualiza nos seus protocolos de curativos os insumos recomendados de forma totalmente dinâmica e integrada:
  ...
  2. *Condições de Exibição dos Parceiros iRec:* Os parceiros iRec oficiais são exibidos de forma completamente isolada em sua própria seção dedicada ("Parceiros iRec"), e não mais misturados dentro dos cards de sugestões da IA ou protocolos padrão.
  3. *Layout Compacto e Remoção de Preços:* Reduzimos significativamente o padding e o espaçamento dos cards de recomendação da IA e protocolos padrão para encurtar a rolagem da página. Adicionalmente, removemos totalmente o campo de valor do insumo recomendado nesses cards.
  4. *Fallback da Farmácia Mais Próxima:* Caso os médicos responsáveis por aquele paciente não tenham parceiros cadastrados, o card exibe uma nota informativa instruindo o paciente a procurar a farmácia credenciada mais próxima para aquisição do produto (substituindo o antigo aviso de consulta ao administrador).
  5. *Modal de Redirecionamento com Contagem Regressiva:* Como a plataforma iRec não realiza vendas diretas, removemos o fluxo de "Pedido Confirmado / iRec Delivery" e o substituímos por uma tela de redirecionamento segura com contagem regressiva de 3 segundos (e redirecionamento automático) que exibe o nome do insumo, o parceiro/farmácia de destino e o nome do médico autor (ou indicação oficial iRec). E-commerce simulado desativado.
